...
public struct ConnectTraxTransport: TraxTransport {
    ...
    private func authHeaders() async throws -> Headers {
        // No token → fail LOCALLY instead of sending an anonymous request the
        // server can only reject (PulseKit's transport has always done this;
        // the Connect port dropped the guard). The host's provider refreshes
        // expired tokens in place, so nil here means "no session yet" — a
        // self-healing state the sync layer debounces rather than banners.
